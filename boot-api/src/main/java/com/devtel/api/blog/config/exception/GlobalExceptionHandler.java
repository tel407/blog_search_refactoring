...
public class GlobalExceptionHandler {
	...
	protected ResponseEntity<ApiErrorResponse> handleMethodArgumentNotValidException(MethodArgumentNotValidException e) {
		...
		final ApiErrorResponse response = ApiErrorResponse.of(ErrorCodeEnum.NOT_VALID_ERROR, e.getBindingResult());
	...
	protected ResponseEntity<ApiErrorResponse> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e) {
		...
		final ApiErrorResponse response = ApiErrorResponse.of(ErrorCodeEnum.METHOD_NOT_ALLOWED);
	...
	protected ResponseEntity<ApiErrorResponse> handleAccessDeniedException(AccessDeniedException e) {
		...
		final ApiErrorResponse response = ApiErrorResponse.of(ErrorCodeEnum.HANDLE_ACCESS_DENIED);
		return new ResponseEntity<>(response, HttpStatus.valueOf(ErrorCodeEnum.HANDLE_ACCESS_DENIED.getStatus()));
	}

	@ExceptionHandler(BusinessException.class)
	protected ResponseEntity<ApiErrorResponse> handleBusinessException(final BusinessException e) {
		final ErrorCodeEnum ErrorCodeEnum = e.getErrorCode();
		final ApiErrorResponse response = ApiErrorResponse.of(ErrorCodeEnum);
		return new ResponseEntity<>(response, HttpStatus.valueOf(ErrorCodeEnum.getStatus()));
	}
	...
	protected ResponseEntity<ApiErrorResponse> handleException(Exception e) {
		...
		final ApiErrorResponse response = ApiErrorResponse.of(ErrorCodeEnum.INTERNAL_SERVER_ERROR);
